...
test("Survey Showdown schedules the next question automatically",()=>{const room={phase:"survey",deadline:1,surveyIndex:0,surveyQuestions:[{},{},{}],mode:"ffa",players:[{id:"a",score:10}],found:new Set(),round:1};__test.finishSurveyQuestion(room);assert.equal(room.phase,"survey-break");assert.ok(room.deadline>Date.now());assert.match(room.message,/automatically/);__test.startNextSurvey(room);assert.equal(room.phase,"survey");assert.equal(room.surveyIndex,1)});
